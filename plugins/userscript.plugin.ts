...
export function generateHeader() {
	 ...
	 */
	if (
		userscript["i18n-names"] &&
		typeof userscript["i18n-names"] === "object"
	) {
	...
	if (packageJson.description || userscript.description) {
		headers.push(
			`// @description ${userscript.description ?? packageJson.description}`,
		);
	}
	// Add userscript header's i18n-descriptions.
	if (
		userscript["i18n-descriptions"] &&
		typeof userscript["i18n-descriptions"] === "object"
	) {
	...
	if (userscript.require && Array.isArray(userscript.require)) {
		for (const require of userscript.require) {
			headers.push(`// @require ${require}`);
		}
	}
	// Add userscript header's resources.
	if (userscript.resources && Array.isArray(userscript.resources)) {
	...
	if (userscript.keyedResources) {
		for (const dependencyName in userscript.keyedResources) {
			headers.push(
				`// @resource ${dependencyName} ${userscript.keyedResources[dependencyName]}`,
			);
		}
	}
	// Add userscript header's connects.
	if (userscript.connect && Array.isArray(userscript.connect)) {
	...
	if (userscript.grant && Array.isArray(userscript.grant)) {
		for (const grant of userscript.grant) {
			headers.push(`// @grant ${grant}`);
		}
	}
	// Add userscript header's antifeatures.
	if (userscript.antifeature && Array.isArray(userscript.antifeature)) {
